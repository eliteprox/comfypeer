...
import { cancelUserSubscription } from "@/lib/billing-plans";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await cancelUserSubscription(owner.externalUserId, {
      timing: body.timing?.trim(),
      effectiveAt: body.effectiveAt?.trim(),
    });
    return NextResponse.json({ ok: true });
  } catch (error) {
    return pmtHouseErrorResponse(error, "Failed to cancel subscription");
  }
}
